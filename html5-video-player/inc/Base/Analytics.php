...
class Analytics
{
    ...
    {
        add_submenu_page('edit.php?post_type=videoplayer', 'Analytics', 'Analytics', 'manage_options', 'analytics', [$this, 'analytics_callback'], 3);
    }

    public function analytics_callback()
    {
        if (isset($_GET['user'])) {
            $id = sanitize_text_field(wp_unslash($_GET['user']));
            $this->analyticsUser($id);
        } else if (isset($_GET['video'])) {
            $id = sanitize_text_field(wp_unslash($_GET['video']));
